...
export function bookingFormRedirect() {
  ...
  form.addEventListener("submit", function (e) {
    ...
    const promoCode =
      form.querySelector('input[name="promo_code"]')?.value.trim() || "";
...
export function closeTripadvidor() {
  if ($(".TA_certificateOfExcellence").length < 1) return;

  const btnClose_TA_certificateOfExcellence = $(
    ".TA_certificateOfExcellence .button-close"
  );
  btnClose_TA_certificateOfExcellence.on("click", function (e) {
    e.preventDefault();
    $(".TA_certificateOfExcellence").animate(
      {
        opacity: 0,
        marginLeft: "-100px"
      },
